describe('Basic user flow for SPA ', () => {
  ...
  it('Test4: On first Entry page - checking page header title', async () => {
    ...
    expect(await page.evaluate(() => document.querySelector('h1').innerText)).toBe("Entry 1")
  });
  ...
  it('Test5: On first Entry page - checking <entry-page> contents', async () => {
    /*
      ...
      */
    const data = {
      title: await page.evaluate(() => document.querySelector('entry-page').shadowRoot.querySelector('.entry-title').innerText),
      date: await page.evaluate(() => document.querySelector('entry-page').shadowRoot.querySelector('.entry-date').innerText),
      content: await page.evaluate(() => document.querySelector('entry-page').shadowRoot.querySelector('.entry-content').innerText),
      image: {
        src: await page.evaluate(() => document.querySelector('entry-page').shadowRoot.querySelector('.entry-image').src),
        alt: await page.evaluate(() => document.querySelector('entry-page').shadowRoot.querySelector('.entry-image').alt),
      }
    }
  ...
  it('Test8: On Settings page - checking page header title', async () => {
    ...
    expect(await page.evaluate(() => document.querySelector('h1').innerText)).toBe("Settings")
  });
  ...
  it('Test11: Clicking the back button once should bring the user back to the home page', async () => {
    await page.goBack();
    expect(page.url()).not.toMatch(/\/#entry1/)
    expect(page.url()).not.toMatch(/\/#settings/)
  });

  // define and implement test12: When the user if on the homepage, the header title should be “Journal Entries”
  it('Test12: When the user if on the homepage, the header title should be “Journal Entries”', async () => {
    expect(await page.evaluate(() => document.querySelector('h1').innerText)).toBe("Journal Entries")
  });

  // define and implement test13: On the home page the <body> element should not have any class attribute 
  it('Test13: On the home page the <body> element should not have any class attribute”', async () => {
    expect(await page.evaluate(() => document.querySelector('body').classList.length)).toBe(0)
  });
  ...
  it('Test19: Verify the title is current when clicking on the third entry', async () => {
    expect(await page.evaluate(() => document.querySelector('h1').innerText)).toBe("Entry 3")
  });
